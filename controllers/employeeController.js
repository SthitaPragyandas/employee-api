...
const updateEmployee = async (req, res) => {
    try {
        ...
        if (!employee) {
            return res.status(404).json({ success: false, error: "employee not found error" })
        }
        ...
        if (!updateEmployee || !updateUser) {
            return res.status(404).json({ success: false, error: "document not found error" })
        }
...
const fetchEmployeesByDepId = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ success: false, error: "get employee by dep id server error" })
    }
}
